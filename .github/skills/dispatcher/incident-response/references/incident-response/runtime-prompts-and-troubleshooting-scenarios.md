...
### Health endpoint and host routing issues

| # | Scenario | Prompt (copy-paste) |
|---|----------|---------------------|
| 11 | **Health check failing after rewrite change** | **After adding a canonical redirect, the configured AMS health-check URL started failing and monitoring marks the instance unhealthy. Trace the configured health-check URL with trace_request and trace one normal content URL. Confirm whether a customer rewrite is intercepting health-check traffic. Validate the vhost/rewrite block and suggest a fix that keeps health checks untouched.** |
| 12 | **Host routing mismatch (author/publish or domain)** | **Requests for a specific hostname now return 404 or redirect to the wrong host. Trace the failing host+URL and a known-good host+URL. List every vhost/rewrite rule that could match. Verify host-to-vhost mapping and author/publish routing intent, then provide exact config change and rollback.** |
