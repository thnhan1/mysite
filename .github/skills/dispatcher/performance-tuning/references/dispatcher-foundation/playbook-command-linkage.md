# Playbook To MCP Command Linkage (AMS Dispatcher)
...
|---|---|---|---|
| Playbook A: New Site Baseline | new site or initial rollout | `validate(dispatcher)` -> `validate(httpd)` -> `lint` -> `sdk(check-files)` -> `sdk(diff-baseline)` -> `trace_request` | `FILT-002`, `RW-001`, `MODE-001` |
| Playbook B: API/Service Endpoint Enablement | API exposure, selectors, method rules | `validate(dispatcher)` -> `validate(httpd)` -> `lint(deep)` -> `sdk(check-files)` -> `trace_request` | `FILT-003`, `FILT-004`, `RW-003` |
| Playbook C: Multi-Site / Multi-Host Routing | host routing or vhost changes | `validate(httpd)` -> `validate(dispatcher)` -> `lint` -> `sdk(check-files)` -> `trace_request` | `RW-001`, `HDR-002`, `MODE-001` |
| Playbook D: Cache Invalidation Tuning | `statfileslevel`, invalidate, flush scope | `validate(dispatcher)` -> `lint(deep)` -> `sdk(check-files)` -> `inspect_cache` -> `trace_request` | `CACHE-003`, `CACHE-004`, `MODE-001` |
...
| Playbook G: SSL/TLS Backend Or Header Contract Change | SSL/TLS, backend header, or proxy behavior | `validate(httpd)` -> `validate(dispatcher)` -> `lint` -> `sdk(check-files)` -> `trace_request` | `HDR-001`, `HDR-002`, `MODE-001` |
| Playbook H: Permission-Sensitive Caching | `/auth_checker` or auth-aware cache logic | **Prereq:** Create auth-check servlet at `/bin/permissioncheck` and allowlist on publish. Then: `validate(dispatcher)` -> `lint(deep)` -> `sdk(check-files)` -> `trace_request` -> `inspect_cache` | `HDR-001`, `CACHE-002`, `CACHE-005` |
| Playbook I: CI/Pre-Deploy Validation Gate | release readiness | `validate(dispatcher)` -> `validate(httpd)` when needed -> `lint(strict)` -> `sdk(check-files)` -> `sdk(diff-baseline)` | `MODE-001`, `MODE-003` |

## Incident Response Playbooks

| Playbook | Use when | MCP command chain | Minimum tests |
|---|---|---|---|
| Playbook 1: 5xx Spike | 5xx surge or backend failure suspicion | `monitor_metrics` -> `tail_logs` -> `trace_request` -> `inspect_cache` -> `validate` -> `lint` | `INC-001`, `INC-002`, `INC-003` |
...
| Playbook 5: Flush Efficiency Review | flush scope or invalidation overhead | `inspect_cache` -> `trace_request` -> `validate(dispatcher)` -> `lint` -> `monitor_metrics` | `CACHE-003`, `CACHE-004` |
